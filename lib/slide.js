...
class Slide {
    constructor(args) {
        ...
        //extract existing shapes, images and connectors
        let shapes = PptxContentHelper.extractSlideObjectInfo(this.content, this.name).shapes;
        let images = PptxContentHelper.extractSlideObjectInfo(this.content, this.name).images;
        let connectors = PptxContentHelper.extractSlideObjectInfo(this.content, this.name).connectors;
        this.elements = (shapes || []).map(shapeContent => {
            let shape = new Shape();
            shape.setContent(shapeContent);
            return shape;
        }) || [];
        this.elements = this.elements.concat((images || []).map(content => {
            let image = new Image();
            image.setContent(content);
            return image;
        }) || []);
        this.elements = this.elements.concat((connectors || []).map(content => {
            let connector = new Connector();
            connector.setContent(content);
            return connector;
        }) || []);
        this.defaults = {};
    }
    ...
    editShape(config, shapeName) {
        try {
            let shape = this.getShapeByName(shapeName);

            this.processConfig(config, shape);

            this.powerPointFactory.editShape(shape);

            return this;
        } catch (err) {
            console.log(err);
            throw new Error(`Failed to edit shape. Exception info: ${err.message}`);
        }
    }
    ...
    copyShape(shapeName, shapeCopyName) {
        try {
            let shape = this.getShapeByName(shapeName);
            let shapeCopy = new Shape();

            this.powerPointFactory.copyShape({ slide: this, shape, shapeCopy, shapeCopyName });

            this.elements.push(shapeCopy);

            return this;
        } catch (err) {
            throw new Error(`Failed to copy shape. Exception info: ${err.message}`);
        }
    }
    ...
    addConnector(config) {
        try {
            ...
            this.processConfig(config, connector);
            this.powerPointFactory.addConnector(this, connector);
            this.elements.push(connector);

            return this;
        } catch (err) {
            throw new Error(`Failed to add connector to slide. Exception info: ${err.message}`);
        }
    }
    ...
    getShapeByName(name) {
        return (this.elements || []).find(element => {
            if(!(element instanceof Shape)) return false;
    ...
    getNextObjectId() {
        let id = this.getNumElements() + 1;
        while(this.elements.find(element => element.id() == id)) id++;
        return id;
    }
}
