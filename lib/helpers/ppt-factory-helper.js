...
class PptFactoryHelper {
    ...
    static createConnectorBlock({ objectId, name, options }) {
        let block = {
            "p:nvCxnSpPr": [
                {
                    "p:cNvPr": [
    ...
    static setShapeBlockName({ shapeBlock, name }) {
        let attributes = Utils.getPath(shapeBlock, ["p:nvSpPr", 0, "p:cNvPr", 0, "$"]);
        if(attributes) attributes.name = name;
    }

    static setShapeBlockId({ shapeBlock, id }) {
        let attributes = Utils.createPath(shapeBlock, ["p:nvSpPr", 0, "p:cNvPr", 0, "$"]);
        if(attributes) attributes.id = id;
    }

    static createBaseShapeBlock({ objectId, name, x, y, cx, cy }) {
        let block = {
            'p:nvSpPr': [
                {
                    'p:cNvPr': [{ $: { id: objectId, name: name } }],
                    'p:cNvSpPr': [{}],
                    'p:nvPr': [{}],
                },
            ],
    ...
    static createColorBlock(color, transparency) {
        ...
        if(transparency) Utils.createPath(colorObject, ["a:srgbClr", 0, "a:alpha", 0, "$"]).val = transparency;
    ...
    static setTextBodyProperties(textBodyPropertyBlock, parentObject, options) {
        ...
        PptFactoryHelper.setOverflowOnTextBody(textBodyPropertyBlock, options);
    }
    ...
    static setOverflowOnTextBody(textBodyPropertyBlock, options) {
        options.overflow ? textBodyPropertyBlock["$"]["vertOverflow"] = options.overflow : delete textBodyPropertyBlock["$"]["vertOverflow"];
    }
    ...
    static addTextValuesToBlock(block, textBox, options) {
        ...
        let textLines;
        let textValues = (textBox.textValue != undefined) ? textBox.textValue : textBox.bulletPoints;
        if(!textValues) textValues = "";

        if (Array.isArray(textValues)) {
            block['a:p'] = [];

            for (let i = 0; i < textValues.length; i++) {
                PptFactoryHelper.addBulletPointsToBlock(block['a:p'], textValues[i], textValues[i], options);
            }
    ...
    static addBulletPointsToBlock(masterParagraphNode, textValue, indentLevel, options) {
        if (typeof textValue === 'string' || typeof textValue === 'number') {
        ...
        } else if (textValue.list) {
            let lines = textValue.textSegments;
            ...
            if (textObject.text !== undefined) {
                PptFactoryHelper.addBulletPointsToBlock(masterParagraphNode, textObject.text, textObject.options.indentSize, textObject.options);
            } else if (textObject.textSegments !== undefined) {
                PptFactoryHelper.createBulletPointAndText(masterParagraphNode, textObject, textObject.options.indentSize, textObject.options);
            }
        }
    }
    ...
    static createBulletPointAndText(masterParagraphNode, textObject, indentLevel, options) {
        ...
        if (bullet === undefined) {
            ...
            if(options.autoNumType) paragraphBlock['a:pPr'][0]['a:buAutoNum'][0]["$"].type = options.autoNumType;
            ...
            if(Object.keys(bullet).length) {
                let listFontOptions = Utils.createPath(paragraphBlock, ['a:pPr', 0, 'a:buFont', 0, "$"]);
                if(bullet.typeface) listFontOptions.typeface = bullet.typeface;
                if(bullet.font) listFontOptions.font = bullet.font;
                if(bullet.pitchFamily) listFontOptions.pitchFamily = bullet.pitchFamily;
                if(bullet.charset) listFontOptions.charset = bullet.charset;
        ...
        if (typeof textValue === 'object') {
            ...
            if (textValue.textSegments !== undefined) {
                for (let segment of textValue.textSegments) {
                    if(segment.list) {
                        PptFactoryHelper.addBulletPointsToBlock(masterParagraphNode, segment, 0, options);
                    } else {
                        let textSegments = segment.textSegments;
                        if(!textSegments && segment.text) {
                            let line = segment.text.replace(/\r*\n/g, CRLF);
                            let splitLines = line.indexOf(CRLF) > -1 ? line.split(CRLF) : [line];
                            textSegments = [];
        
                            splitLines.forEach(line => {
                                if (line) textSegments.push(Object.assign(segment, { text: line }));
                            });
                        }
                        segment.textSegments = textSegments || [];
                        PptFactoryHelper.createMultiFormattedText(masterParagraphNode, segment, options);
                    }
                }
            }
        }
